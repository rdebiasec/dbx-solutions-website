export const ptEntries = [
  [
    "Home",
    "Início"
  ],
  [
    "Solutions",
    "Soluções"
  ],
  [
    "Services",
    "Serviços"
  ],
  [
    "Industries",
    "Setores"
  ],
  [
    "About",
    "Sobre"
  ],
  [
    "Contact",
    "Contato"
  ],
  [
    "Book a Consultation",
    "Marque uma consulta"
  ],
  [
    "Book a Free Consultation",
    "Agende uma consulta grátis"
  ],
  [
    "Explore Solutions",
    "Explorar soluções"
  ],
  [
    "Explore service",
    "Explorar serviço"
  ],
  [
    "View Services",
    "Ver serviços"
  ],
  [
    "View All Services",
    "Ver todos os serviços"
  ],
  [
    "Explore Industries",
    "Explorar setores"
  ],
  [
    "Contact Us",
    "Contate-nos"
  ],
  [
    "Email Us",
    "Envie-nos um e-mail"
  ],
  [
    "Next Step",
    "Próxima etapa"
  ],
  [
    "Start Practical",
    "Comece de forma prática"
  ],
  [
    "DBX Solutions | WhatsApp & AI Customer Experience for SMBs",
    "Soluções DBX | Experiência do cliente WhatsApp e IA para pequenas e médias empresas"
  ],
  [
    "DBX Solutions | AI-Powered Customer Experience for SMBs",
    "Soluções DBX | Experiência do cliente baseada em IA para pequenas e médias empresas"
  ],
  [
    "AI-Powered Customer Experience",
    "Experiência do cliente baseada em IA"
  ],
  [
    "AI-Powered Customer Experience for Growing SMBs",
    "Experiência do cliente baseada em IA para pequenas e médias empresas em crescimento"
  ],
  [
    "DBX Solutions helps small and mid-sized businesses automate customer conversations, improve response times, and create smarter customer journeys without adding operational complexity.",
    "A DBX Solutions ajuda pequenas e médias empresas a automatizar as conversas com os clientes, melhorar os tempos de resposta e criar jornadas mais inteligentes para os clientes, sem adicionar complexidade operacional."
  ],
  [
    "Practical AI solutions designed for real business operations, not generic automation.",
    "Soluções práticas de IA projetadas para operações comerciais reais, não para automação genérica."
  ],
  [
    "Respond faster to customers",
    "Responda mais rápido aos clientes"
  ],
  [
    "Reduce repetitive manual work",
    "Reduza o trabalho manual repetitivo"
  ],
  [
    "Capture and qualify more leads",
    "Capture e qualifique mais leads"
  ],
  [
    "Connect conversations with business tools",
    "Conecte conversas com ferramentas de negócios"
  ],
  [
    "The Challenge",
    "O desafio"
  ],
  [
    "Customer expectations are rising. Manual processes are slowing businesses down.",
    "As expectativas dos clientes estão aumentando. Os processos manuais estão atrasando as empresas."
  ],
  [
    "Many growing businesses rely on small teams, disconnected tools, and manual follow-ups to manage customer conversations. As demand increases, response times slow down, leads get missed, and teams spend too much time on repetitive tasks.",
    "Muitas empresas em crescimento dependem de equipes pequenas, ferramentas desconectadas e acompanhamento manual para gerenciar as conversas com os clientes. À medida que a demanda aumenta, os tempos de resposta diminuem, leads são perdidos e as equipes gastam muito tempo em tarefas repetitivas."
  ],
  [
    "Slow Response Times",
    "Tempos de resposta lentos"
  ],
  [
    "Customers expect quick answers. When teams are busy, delayed responses can lead to missed opportunities and lower satisfaction.",
    "Os clientes esperam respostas rápidas. Quando as equipes estão ocupadas, as respostas atrasadas podem levar à perda de oportunidades e à menor satisfação."
  ],
  [
    "Repetitive Customer Questions",
    "Perguntas repetitivas do cliente"
  ],
  [
    "Teams spend valuable time answering the same questions across email, chat, forms, and other customer communication channels.",
    "As equipes gastam um tempo valioso respondendo às mesmas perguntas por e-mail, chat, formulários e outros canais de comunicação com o cliente."
  ],
  [
    "Missed Leads and Follow-Ups",
    "Leads e acompanhamentos perdidos"
  ],
  [
    "Without automated qualification and routing, interested prospects can fall through the cracks before the team responds.",
    "Sem qualificação e roteamento automatizados, os clientes em potencial interessados podem passar despercebidos antes que a equipe responda."
  ],
  [
    "Disconnected Tools",
    "Ferramentas desconectadas"
  ],
  [
    "Customer conversations, CRM data, support requests, and internal workflows are often spread across systems that do not work smoothly together.",
    "Conversas com clientes, dados de CRM, solicitações de suporte e fluxos de trabalho internos geralmente estão espalhados por sistemas que não funcionam perfeitamente juntos."
  ],
  [
    "Limited Team Capacity",
    "Capacidade limitada da equipe"
  ],
  [
    "Growing businesses need better customer experience, but hiring more people is not always the fastest or most efficient solution.",
    "As empresas em crescimento precisam de uma melhor experiência do cliente, mas contratar mais pessoas nem sempre é a solução mais rápida ou eficiente."
  ],
  [
    "The Solution",
    "A solução"
  ],
  [
    "Smarter customer conversations. Simpler business operations.",
    "Conversas mais inteligentes com os clientes. Operações comerciais mais simples."
  ],
  [
    "DBX Solutions designs and implements AI-powered customer experience systems that help businesses respond faster, support customers more consistently, and streamline the way work moves across teams and tools.",
    "A DBX Solutions projeta e implementa sistemas de experiência do cliente baseados em IA que ajudam as empresas a responder mais rapidamente, oferecer suporte aos clientes de forma mais consistente e agilizar a forma como o trabalho se move entre equipes e ferramentas."
  ],
  [
    "Instead of adding more complexity, we help your business use AI where it creates real operational value: customer communication, lead qualification, support automation, workflow integration, and process improvement.",
    "Em vez de adicionar mais complexidade, ajudamos sua empresa a usar IA onde ela cria valor operacional real: comunicação com o cliente, qualificação de leads, automação de suporte, integração de fluxo de trabalho e melhoria de processos."
  ],
  [
    "Conversational AI",
    "IA conversacional"
  ],
  [
    "Agentic AI that answers questions, takes action, collects information, and supports conversations across digital channels.",
    "IA agéntica que responde perguntas, age, coleta informações e apoia conversas em canais digitais."
  ],
  [
    "Customer Experience Automation",
    "Automação da experiência do cliente"
  ],
  [
    "Automated flows that improve speed, consistency, and service quality across common customer interactions.",
    "Fluxos automatizados que melhoram a velocidade, a consistência e a qualidade do serviço nas interações comuns com os clientes."
  ],
  [
    "Workflow Integration",
    "Integração de fluxo de trabalho"
  ],
  [
    "Connected systems that move customer information between tools, teams, and processes more efficiently.",
    "Sistemas conectados que movem informações de clientes entre ferramentas, equipes e processos com mais eficiência."
  ],
  [
    "AI Strategy and Implementation",
    "Estratégia e implementação de IA"
  ],
  [
    "Practical guidance and implementation support to help your business adopt AI with clarity, control, and measurable purpose.",
    "Orientação prática e suporte de implementação para ajudar sua empresa a adotar IA com clareza, controle e propósito mensurável."
  ],
  [
    "AI solutions designed around your customer journey",
    "Soluções de IA projetadas em torno da jornada do cliente"
  ],
  [
    "Every business has different customer touchpoints, tools, and operational challenges. DBX Solutions helps identify where AI can make the greatest impact, then designs solutions that fit your workflows, team capacity, and business goals.",
    "Cada empresa tem diferentes pontos de contato com o cliente, ferramentas e desafios operacionais. A DBX Solutions ajuda a identificar onde a IA pode gerar o maior impacto e, em seguida, projeta soluções que se adaptam aos seus fluxos de trabalho, capacidade da equipe e objetivos de negócios."
  ],
  [
    "Agentic AI for Customers",
    "IA agéntica para clientes"
  ],
  [
    "AI Customer Assistants",
    "Assistentes de cliente de IA"
  ],
  [
    "Agentic AI for WhatsApp, web chat, SMS, forms, and email that takes action—answers, qualifies, collects data, and routes follow-up. Not a scripted FAQ chatbot.",
    "IA agéntica para WhatsApp, chat na web, SMS, formulários e e-mail que age—responde, qualifica, coleta dados e encaminha o acompanhamento. Não é um chatbot de FAQ com respostas pré-definidas."
  ],
  [
    "Create intelligent assistants that answer common questions, guide customers, collect information, and support your team across digital channels.",
    "Crie assistentes inteligentes que respondem a perguntas comuns, orientam clientes, coletam informações e apoiam sua equipe em canais digitais."
  ],
  [
    "Lead Qualification Automation",
    "Automação de qualificação de leads"
  ],
  [
    "Use AI-powered conversation flows to capture prospect details, qualify interest, ask the right questions, and route leads to the right next step.",
    "Utilize fluxos de conversa com IA para capturar detalhes de prospects, qualificar interesse, fazer as perguntas certas e encaminhar leads para a próxima etapa."
  ],
  [
    "Customer Support Automation",
    "Automação de suporte ao cliente"
  ],
  [
    "Automate common support interactions, frequently asked questions, intake forms, ticket routing, and customer updates.",
    "Automatize interações comuns de suporte, perguntas frequentes, formulários de captação, encaminhamento de tickets e atualizações de clientes."
  ],
  [
    "CRM and Workflow Integration",
    "Integração de CRM e fluxo de trabalho"
  ],
  [
    "AI Strategy and Roadmapping",
    "Estratégia e roteiro de IA"
  ],
  [
    "Optimization and Continuous Improvement",
    "Otimização e Melhoria Contínua"
  ],
  [
    "Best for",
    "Ideal para"
  ],
  [
    "Business outcome",
    "Resultado do negócio"
  ],
  [
    "Business Impact",
    "Impacto nos negócios"
  ],
  [
    "Built to improve customer experience without overwhelming your team",
    "Criado para melhorar a experiência do cliente sem sobrecarregar sua equipe"
  ],
  [
    "Faster Response Times",
    "Tempos de resposta mais rápidos"
  ],
  [
    "Fewer Missed Opportunities",
    "Menos oportunidades perdidas"
  ],
  [
    "Reduced Manual Workload",
    "Carga de trabalho manual reduzida"
  ],
  [
    "Better Customer Satisfaction",
    "Melhor satisfação do cliente"
  ],
  [
    "More Consistent Communication",
    "Comunicação mais consistente"
  ],
  [
    "Scalable Support",
    "Suporte escalável"
  ],
  [
    "Improved Operational Visibility",
    "Visibilidade Operacional Melhorada"
  ],
  [
    "Practical AI Adoption",
    "Adoção prática de IA"
  ],
  [
    "Our Process",
    "Nosso Processo"
  ],
  [
    "A simple path from AI idea to business impact",
    "Um caminho simples da ideia de IA ao impacto nos negócios"
  ],
  [
    "Discover",
    "Descobrir"
  ],
  [
    "Design",
    "Concepção"
  ],
  [
    "Implement",
    "Implementar"
  ],
  [
    "Optimize",
    "Otimizar"
  ],
  [
    "Use Cases",
    "Casos de uso"
  ],
  [
    "AI customer experience solutions for service-driven businesses",
    "Soluções de experiência do cliente de IA para empresas orientadas a serviços"
  ],
  [
    "Professional Services",
    "Serviços Profissionais"
  ],
  [
    "Healthcare and Wellness Clinics",
    "Clínicas de saúde e bem-estar"
  ],
  [
    "Real Estate Teams",
    "Equipes imobiliárias"
  ],
  [
    "Local Service Businesses",
    "Empresas de serviços locais"
  ],
  [
    "E-Commerce Businesses",
    "Empresas de comércio eletrônico"
  ],
  [
    "Education and Training Providers",
    "Provedores de educação e treinamento"
  ],
  [
    "Financial and Advisory Services",
    "Serviços Financeiros e de Consultoria"
  ],
  [
    "Why DBX Solutions",
    "Por que a DBX Solutions"
  ],
  [
    "Practical AI implementation with a human-centered approach",
    "Implementação prática de IA com uma abordagem centrada no ser humano"
  ],
  [
    "SMB-Focused",
    "Focado em pequenas e médias empresas"
  ],
  [
    "Business-First Approach",
    "Abordagem que prioriza os negócios"
  ],
  [
    "Human Oversight",
    "Supervisão Humana"
  ],
  [
    "Integration-Aware",
    "Integração desde o início"
  ],
  [
    "Outcome-Oriented",
    "Orientado para resultados"
  ],
  [
    "Built to Improve",
    "Construído para melhorar"
  ],
  [
    "Questions businesses ask before adopting AI",
    "Perguntas que as empresas fazem antes de adotar a IA"
  ],
  [
    "Ready to improve your customer experience with practical AI?",
    "Pronto para melhorar a experiência do cliente com IA prática?"
  ],
  [
    "Let’s identify where automation can help your business respond faster, serve customers better, and operate more efficiently.",
    "Vamos identificar onde a automação pode ajudar sua empresa a responder mais rapidamente, atender melhor os clientes e operar com mais eficiência."
  ],
  [
    "No generic AI pitch. Just a focused conversation about your business, your customer journey, and where AI can create real value.",
    "Nenhum argumento genérico de IA. Apenas uma conversa focada sobre seu negócio, a jornada do cliente e onde a IA pode criar valor real."
  ],
  [
    "Build a smarter customer experience without overwhelming your team.",
    "Crie uma experiência de cliente mais inteligente sem sobrecarregar sua equipe."
  ],
  [
    "Solutions that make AI practical for your business",
    "Soluções que tornam a IA prática para seu negócio"
  ],
  [
    "Helping SMBs turn AI into better customer experiences",
    "Ajudando pequenas e médias empresas a transformar IA em melhores experiências para o cliente"
  ],
  [
    "Let’s explore how AI can improve your customer experience",
    "Vamos explorar como a IA pode melhorar a experiência do cliente"
  ],
  [
    "Request Consultation",
    "Solicitar Consulta"
  ],
  [
    "First name",
    "Primeiro nome"
  ],
  [
    "Last name",
    "Sobrenome"
  ],
  [
    "Business email",
    "E-mail comercial"
  ],
  [
    "Company name",
    "Nome da empresa"
  ],
  [
    "Phone number",
    "Número de telefone"
  ],
  [
    "Website",
    "Site"
  ],
  [
    "What are you looking to improve?",
    "O que você busca melhorar?"
  ],
  [
    "Select one",
    "Selecione um"
  ],
  [
    "Customer response times",
    "Tempos de resposta do cliente"
  ],
  [
    "Workflow integration",
    "Integração de fluxo de trabalho"
  ],
  [
    "AI strategy",
    "Estratégia de IA"
  ],
  [
    "Not sure yet",
    "Ainda não tenho certeza"
  ],
  [
    "Message",
    "Mensagem"
  ],
  [
    "Book directly on the calendar",
    "Reserve diretamente no calendário"
  ],
  [
    "Thank you. Your request has been captured for follow-up.",
    "Obrigado. Sua solicitação foi registrada para acompanhamento."
  ],
  [
    "Privacy Policy",
    "Política de Privacidade"
  ],
  [
    "Terms of Service",
    "Termos de Serviço"
  ],
  [
    "Customer message",
    "Mensagem do cliente"
  ],
  [
    "DBX AI Agent",
    "Agente de IA da DBX"
  ],
  [
    "DBX Agent",
    "Agente DBX"
  ],
  [
    "AI assistant",
    "Assistente de IA"
  ],
  [
    "DBX AI Assistant",
    "Assistente de IA da DBX"
  ],
  [
    "DBX Assistant",
    "Assistente DBX"
  ],
  [
    "Workflow automation",
    "Automação de fluxo de trabalho"
  ],
  [
    "CRM / team follow-up",
    "Acompanhamento de CRM/equipe"
  ],
  [
    "Incoming",
    "Entrada"
  ],
  [
    "Team handoff",
    "Transferência de equipe"
  ],
  [
    "Qualify, answer, route",
    "Qualificar, responder, encaminhar"
  ],
  [
    "Lead summary sent with next step",
    "Resumo do lead enviado com a próxima etapa"
  ],
  [
    "WhatsApp & Customer Channels",
    "WhatsApp e canais de clientes"
  ],
  [
    "Respond faster on WhatsApp and connect every conversation to your team—without hiring more staff.",
    "Responda mais rápido no WhatsApp e conecte todas as conversas à sua equipe, sem contratar mais funcionários."
  ],
  [
    "DBX implements and manages agentic AI for growing businesses—systems that respond, capture lead details, trigger follow-up in your CRM or team, and escalate when judgment matters. Not a traditional chatbot that only answers FAQs.",
    "A DBX implementa e opera IA agéntica para empresas em crescimento—sistemas que respondem, capturam dados de leads, acionam o acompanhamento no CRM ou na equipe e escalam quando o critério humano importa. Não é um chatbot tradicional que só responde perguntas frequentes."
  ],
  [
    "WhatsApp, SMS, web chat, and email",
    "WhatsApp, SMS, chat na web e e-mail"
  ],
  [
    "Faster first response, including after hours",
    "Primeira resposta mais rápida, inclusive após o expediente"
  ],
  [
    "Your team steps in when judgment matters",
    "Sua equipe entra quando o critério humano importa"
  ],
  [
    "Results",
    "Resultados"
  ],
  [
    "Practical improvements businesses can measure",
    "Melhorias práticas que as empresas podem medir"
  ],
  [
    "DBX focuses on agentic AI that takes action—response speed, cleaner intake, and follow-up your team can use. Not traditional chatbot demos.",
    "A DBX foca em IA agéntica que age—velocidade de resposta, captação mais clara e acompanhamento que sua equipe pode usar. Não são demonstrações de chatbot tradicional."
  ],
  [
    "Free consultations include a channel review and three practical agentic AI opportunities.",
    "As consultas gratuitas incluem uma análise dos canais e três oportunidades práticas de IA agéntica."
  ],
  [
    "Under 15 minutes",
    "Menos de 15 minutos"
  ],
  [
    "Typical first-response improvement on WhatsApp and chat after launch.",
    "Melhoria típica de primeira resposta no WhatsApp e chat após o lançamento."
  ],
  [
    "Structured intake",
    "Captação estruturada"
  ],
  [
    "Lead details captured before your team picks up the conversation.",
    "Detalhes do lead capturados antes que sua equipe inicie a conversa."
  ],
  [
    "Human oversight",
    "Supervisão humana"
  ],
  [
    "Escalation paths keep your team in control of important decisions.",
    "Os caminhos de escalonamento mantêm sua equipe no controle de decisões importantes."
  ],
  [
    "DBX helped us organize WhatsApp inquiries and give the team useful context before follow-up. Responses got faster without adding headcount.",
    "A DBX nos ajudou a organizar as consultas do WhatsApp e a fornecer à equipe um contexto útil antes do acompanhamento. As respostas ficaram mais rápidas sem aumentar o número de funcionários."
  ],
  [
    "Operations Lead",
    "Líder de Operações"
  ],
  [
    "U.S. services business",
    "Negócio de serviços nos EUA"
  ],
  [
    "Free consultation includes a review of your customer channels and three practical automation opportunities.",
    "A consulta gratuita inclui uma análise dos canais do seu cliente e três oportunidades práticas de automação."
  ],
  [
    "Identify the first WhatsApp workflow worth improving.",
    "Identifique o primeiro fluxo de trabalho do WhatsApp que vale a pena melhorar."
  ],
  [
    "Bring your current channels, common questions, and follow-up process. We will show you where response, intake, or qualification should become clearer first.",
    "Traga seus canais atuais, perguntas comuns e processo de acompanhamento. Mostraremos onde a resposta, a captação ou a qualificação devem ficar mais claras primeiro."
  ],
  [
    "Every consultation includes three practical automation opportunities for your business.",
    "Cada consulta inclui três oportunidades práticas de automação para seu negócio."
  ],
  [
    "Review My WhatsApp Workflow",
    "Revise meu fluxo de trabalho do WhatsApp"
  ],
  [
    "A focused 30-minute consultation reviews your customer journey, tools, and the operational gap costing you leads or response time.",
    "Uma consulta focada de 30 minutos analisa a jornada do cliente, as ferramentas e a lacuna operacional que custa leads ou tempo de resposta."
  ],
  [
    "Start with one workflow, then expand based on what works.",
    "Comece com um fluxo de trabalho e depois expanda com base no que funciona."
  ],
  [
    "Book a 30-Minute Consultation",
    "Agende uma consulta de 30 minutos"
  ],
  [
    "Ready to improve WhatsApp response and lead follow-up?",
    "Pronto para melhorar a resposta do WhatsApp e o acompanhamento de leads?"
  ],
  [
    "Book a free consultation. We will review your channels, identify the first workflow to improve, and outline three practical agentic AI opportunities for your business.",
    "Agende uma consulta gratuita. Analisaremos seus canais, identificaremos o primeiro fluxo de trabalho a melhorar e delinearemos três oportunidades práticas de IA agéntica para seu negócio."
  ],
  [
    "DBX Solutions helps growing businesses improve customer conversations on WhatsApp and other channels, qualify leads, reduce repetitive work, and connect AI with real operations.",
    "A DBX Solutions ajuda empresas em crescimento a melhorar as conversas com os clientes no WhatsApp e outros canais, qualificar leads, reduzir o trabalho repetitivo e conectar a IA com operações reais."
  ],
  [
    "DBX Solutions helps growing businesses respond faster on WhatsApp, qualify leads, automate repetitive conversations, and connect customer interactions with the tools their teams already use.",
    "A DBX Solutions ajuda empresas em crescimento a responder mais rapidamente no WhatsApp, qualificar leads, automatizar conversas repetitivas e conectar interações com clientes com as ferramentas que suas equipes já usam."
  ],
  [
    "Book a consultation or send a quick request.",
    "Marque uma consulta ou envie uma solicitação rápida."
  ],
  [
    "Prefer the fastest path? Use the calendar. Only need a short note? The form takes under a minute.",
    "Prefere o caminho mais rápido? Use o calendário. Precisa apenas de uma breve nota? O formulário leva menos de um minuto."
  ],
  [
    "Name",
    "Nome"
  ],
  [
    "Your name",
    "Seu nome"
  ],
  [
    "Phone or WhatsApp",
    "Telefone ou WhatsApp"
  ],
  [
    "(optional)",
    "(opcional)"
  ],
  [
    "Share your main channel, common questions, or follow-up challenge.",
    "Compartilhe seu canal principal, perguntas comuns ou desafio de acompanhamento."
  ],
  [
    "Let’s review where WhatsApp and customer conversations are costing you time or leads",
    "Vamos analisar onde as conversas do WhatsApp e dos clientes estão custando tempo ou leads"
  ],
  [
    "Book on the calendar for the fastest response, or send a short request below. We will help identify the most practical first workflow to improve.",
    "Reserve no calendário para obter uma resposta mais rápida ou envie uma breve solicitação abaixo. Ajudaremos a identificar o primeiro fluxo de trabalho mais prático a ser melhorado."
  ],
  [
    "DBX Solutions helps growing businesses turn WhatsApp and customer conversations into faster responses, qualified leads, and connected follow-up workflows.",
    "A DBX Solutions ajuda empresas em crescimento a transformar conversas do WhatsApp e de clientes em respostas mais rápidas, leads qualificados e fluxos de trabalho de acompanhamento conectados."
  ],
  [
    "Practical AI for customer experience and operations.",
    "IA prática para experiência e operações do cliente."
  ],
  [
    "Book a consultation with DBX Solutions to review WhatsApp, lead intake, support automation, and practical AI opportunities for your business.",
    "Agende uma consulta com a DBX Solutions para avaliar o WhatsApp, captação de leads, automação de suporte e oportunidades práticas de IA para seu negócio."
  ],
  [
    "Can you check my order status and send the latest delivery update?",
    "Você pode verificar o status do meu pedido e enviar a última atualização de entrega?"
  ],
  [
    "Can you check my order status?",
    "Você pode verificar o status do meu pedido?"
  ],
  [
    "I found your order. It is packed, assigned to delivery, and expected today between 2pm and 4pm.",
    "Encontrei seu pedido. Está embalado, designado para entrega e previsto para hoje entre 14h e 16h."
  ],
  [
    "Your order is packed and expected today between 2pm and 4pm.",
    "Seu pedido está embalado e esperado hoje entre 14h e 16h."
  ],
  [
    "Can you notify my team too?",
    "Você pode notificar minha equipe também?"
  ],
  [
    "Done. I synced the update to your CRM and sent the team alert with the customer note.",
    "Feito. Sincronizei a atualização com o seu CRM e enviei o alerta da equipe com a nota do cliente."
  ],
  [
    "Done. I synced the update to your CRM and alerted your team.",
    "Feito. Sincronizei a atualização com seu CRM e alertei sua equipe."
  ],
  [
    "Perfect, thank you.",
    "Perfeito, obrigado."
  ],
  [
    "You are welcome. I will watch for delivery changes and update the record if anything shifts.",
    "De nada. Ficarei atento às alterações na entrega e atualizarei o registro se alguma coisa mudar."
  ],
  [
    "You are welcome. I will watch for delivery changes.",
    "De nada. Estarei atento às alterações na entrega."
  ],
  [
    "online now",
    "online agora"
  ],
  [
    "online",
    "online"
  ],
  [
    "add",
    "adicionar"
  ],
  [
    "Today",
    "Hoje"
  ],
  [
    "End-to-end encrypted",
    "Criptografado de ponta a ponta"
  ],
  [
    "WhatsApp voice call",
    "Chamada de voz WhatsApp"
  ],
  [
    "Audio",
    "Áudio"
  ],
  [
    "Video",
    "Vídeo"
  ],
  [
    "Mute",
    "Silenciar"
  ],
  [
    "End",
    "Encerrar"
  ],
  [
    "mobile 00:51",
    "celular 00:51"
  ],
  [
    "mute",
    "mudo"
  ],
  [
    "keypad",
    "teclado"
  ],
  [
    "speaker",
    "Alto-falante"
  ],
  [
    "add call",
    "adicionar chamada"
  ],
  [
    "contacts",
    "contatos"
  ],
  [
    "From answering repetitive questions to chasing down leads, we take the \"busy\" out of business. It’s enterprise-grade automation that actually feels like help.",
    "De responder perguntas repetitivas a acompanhar leads, tiramos a \"correria\" do dia a dia. É automação de nível empresarial que realmente ajuda."
  ],
  [
    "See How It Works",
    "Veja como funciona"
  ],
  [
    "Customer support automation",
    "Automação de suporte ao cliente"
  ],
  [
    "Lead engagement and qualification",
    "Engajamento e qualificação de leads"
  ],
  [
    "Workflow and CRM integration",
    "Integração de fluxo de trabalho e CRM"
  ],
  [
    "Human-supervised AI operations",
    "Operações de IA supervisionadas por humanos"
  ],
  [
    "Slow responses",
    "Respostas lentas"
  ],
  [
    "Customers move on when questions sit unanswered during busy hours or after closing.",
    "Os clientes seguem em frente quando as perguntas ficam sem resposta durante os horários de pico ou após o fechamento."
  ],
  [
    "Repetitive questions",
    "Perguntas repetitivas"
  ],
  [
    "Your team spends time repeating answers that could be handled consistently.",
    "Sua equipe gasta tempo repetindo respostas que poderiam ser tratadas de forma consistente."
  ],
  [
    "Missed leads",
    "Leads perdidos"
  ],
  [
    "High-intent inquiries lose momentum when follow-up is delayed or incomplete.",
    "As consultas de alta intenção perdem força quando o acompanhamento é atrasado ou incompleto."
  ],
  [
    "Disconnected tools",
    "Ferramentas desconectadas"
  ],
  [
    "Conversation details often stay separate from the CRM, calendar, support desk, or team workflow.",
    "Os detalhes da conversa geralmente ficam separados do CRM, do calendário, da central de suporte ou do fluxo de trabalho da equipe."
  ],
  [
    "Limited team capacity",
    "Capacidade limitada da equipe"
  ],
  [
    "Growing demand creates more customer conversations than a lean team can manage manually.",
    "A demanda crescente cria mais conversas com os clientes do que uma equipe enxuta pode gerenciar manualmente."
  ],
  [
    "Customer experience automation connected to real business operations.",
    "Automação da experiência do cliente conectada a operações comerciais reais."
  ],
  [
    "DBX Solutions is not just another AI chat layer. We design customer conversation systems that collect the right information, trigger the right workflow, and give your team useful context.",
    "DBX Solutions não é apenas mais uma camada de chat de IA. Projetamos sistemas de conversação com o cliente que coletam as informações corretas, acionam o fluxo de trabalho correto e fornecem contexto útil à sua equipe."
  ],
  [
    "The result is a practical operations layer for customer support, lead engagement, workflow automation, and smarter follow-up.",
    "O resultado é uma camada de operações práticas para suporte ao cliente, envolvimento de leads, automação de fluxo de trabalho e acompanhamento mais inteligente."
  ],
  [
    "Customer Channels",
    "Canais de clientes"
  ],
  [
    "WhatsApp, web chat, SMS, forms, and email.",
    "WhatsApp, chat na web, SMS, formulários e e-mail."
  ],
  [
    "WhatsApp",
    "WhatsApp"
  ],
  [
    "Web chat",
    "Bate-papo na web"
  ],
  [
    "Forms",
    "Formulários"
  ],
  [
    "Email",
    "E-mail"
  ],
  [
    "Answers, qualifies, collects information, and routes each conversation.",
    "Responde, qualifica, coleta informações e encaminha cada conversa."
  ],
  [
    "Answer",
    "Responder"
  ],
  [
    "Qualify",
    "Qualificar"
  ],
  [
    "Collect",
    "Coletar"
  ],
  [
    "Route",
    "Encaminhar"
  ],
  [
    "Business Systems",
    "Sistemas de Negócios"
  ],
  [
    "Connects customer context to your CRM, calendar, support desk, and team notifications.",
    "Conecta o contexto do cliente ao seu CRM, calendário, central de atendimento e notificações da equipe."
  ],
  [
    "Calendar",
    "Calendário"
  ],
  [
    "Support desk",
    "Balcão de suporte"
  ],
  [
    "Team notifications",
    "Notificações da equipe"
  ],
  [
    "Human Team",
    "Equipe Humana"
  ],
  [
    "Your team reviews, follows up, closes opportunities, and supports customers when human judgment matters.",
    "Sua equipe analisa, acompanha, fecha oportunidades e apoia os clientes quando o critério humano é importante."
  ],
  [
    "Review",
    "Revisar"
  ],
  [
    "Follow up",
    "Acompanhar"
  ],
  [
    "Close",
    "Fechar"
  ],
  [
    "Support",
    "Apoiar"
  ],
  [
    "Managed AI Operations",
    "Operações gerenciadas de IA"
  ],
  [
    "Launch is only the beginning.",
    "O lançamento é apenas o começo."
  ],
  [
    "AI systems need ongoing review, tuning, and improvement. DBX helps keep your agentic AI accurate, useful, and aligned with your business as customer behavior and workflows change.",
    "Os sistemas de IA precisam de revisão, ajuste e melhoria contínuas. O DBX ajuda a manter sua IA agéntica precisa, útil e alinhada com seus negócios à medida que o comportamento do cliente e os fluxos de trabalho mudam."
  ],
  [
    "Conversation quality review",
    "Revisão da qualidade da conversa"
  ],
  [
    "Workflow performance monitoring",
    "Monitoramento de desempenho do fluxo de trabalho"
  ],
  [
    "AI response improvement",
    "Melhoria da resposta de IA"
  ],
  [
    "New automation opportunities",
    "Novas oportunidades de automação"
  ],
  [
    "Operational improvements your team and customers can feel.",
    "Melhorias operacionais que sua equipe e clientes podem sentir."
  ],
  [
    "Faster response times",
    "Tempos de resposta mais rápidos"
  ],
  [
    "Fewer missed opportunities",
    "Menos oportunidades perdidas"
  ],
  [
    "Reduced manual workload",
    "Carga de trabalho manual reduzida"
  ],
  [
    "More consistent customer communication",
    "Comunicação mais consistente com o cliente"
  ],
  [
    "Better operational visibility",
    "Melhor visibilidade operacional"
  ],
  [
    "Scalable support without immediate headcount growth",
    "Suporte escalável sem crescimento imediato do número de funcionários"
  ],
  [
    "High-priority SMB sectors",
    "Setores de pequenas e médias empresas de alta prioridade"
  ],
  [
    "Additional sectors",
    "Setores adicionais"
  ],
  [
    "Professional services",
    "Serviços profissionais"
  ],
  [
    "Healthcare and wellness clinics",
    "Clínicas de saúde e bem-estar"
  ],
  [
    "Real estate teams",
    "Equipes imobiliárias"
  ],
  [
    "Local service businesses",
    "Empresas de serviços locais"
  ],
  [
    "E-commerce",
    "Comércio eletrônico"
  ],
  [
    "Education and training",
    "Educação e treinamento"
  ],
  [
    "Financial and advisory services",
    "Serviços financeiros e de consultoria"
  ],
  [
    "A practical partner for customer experience systems.",
    "Um parceiro prático para sistemas de experiência do cliente."
  ],
  [
    "We do not just launch agentic AI. We design, connect, monitor, and improve customer experience systems around your real business operations.",
    "Não lançamos apenas IA agéntica. Projetamos, conectamos, monitoramos e melhoramos sistemas de experiência do cliente em torno de suas operações comerciais reais."
  ],
  [
    "We do not just launch AI assistants. We design, connect, monitor, and improve customer experience systems around your real business operations.",
    "Não lançamos apenas assistentes de IA. Projetamos, conectamos, monitoramos e melhoramos sistemas de experiência do cliente em torno de suas operações comerciais reais."
  ],
  [
    "SMB-focused implementation",
    "Implementação focada em pequenas e médias empresas"
  ],
  [
    "Business-first design",
    "Design que prioriza os negócios"
  ],
  [
    "Human-supervised AI",
    "IA supervisionada por humanos"
  ],
  [
    "Integration-aware solutions",
    "Soluções pensadas para integração"
  ],
  [
    "Outcome-oriented delivery",
    "Entrega orientada para resultados"
  ],
  [
    "Continuous improvement",
    "Melhoria contínua"
  ],
  [
    "Can this work with WhatsApp?",
    "Isso pode funcionar com o WhatsApp?"
  ],
  [
    "Can humans take over conversations?",
    "Os humanos podem assumir o controle das conversas?"
  ],
  [
    "How are AI responses controlled?",
    "Como as respostas da IA são controladas?"
  ],
  [
    "What does DBX Solutions do?",
    "O que a DBX Solutions faz?"
  ],
  [
    "How long does implementation take?",
    "Quanto tempo leva a implementação?"
  ],
  [
    "Will AI replace our customer service team?",
    "A IA substituirá nossa equipe de atendimento ao cliente?"
  ],
  [
    "Start With a Practical AI Consultation",
    "Comece com uma consulta prática de IA"
  ],
  [
    "Start with a practical AI consultation.",
    "Comece com uma consulta prática de IA."
  ],
  [
    "LinkedIn",
    "LinkedIn"
  ],
  [
    "Serving SMBs remotely across the U.S. and Latin America.",
    "Atendendo pequenas e médias empresas remotamente nos EUA e na América Latina."
  ],
  [
    "Serving businesses remotely across the U.S. and Latin America.",
    "Atendendo empresas remotamente nos EUA e na América Latina."
  ],
  [
    "Verified Accuracy",
    "Precisão verificada"
  ],
  [
    "Answers grounded in your specific business data for total precision.",
    "Respostas baseadas em seus dados comerciais específicos para total precisão."
  ],
  [
    "Unified Channels",
    "Canais Unificados"
  ],
  [
    "Connect everywhere: WhatsApp, SMS, Web, and Email in one place.",
    "Conecte-se em qualquer lugar: WhatsApp, SMS, Web e e-mail em um só lugar."
  ],
  [
    "Integrated Workflows",
    "Fluxos de trabalho integrados"
  ],
  [
    "Instant sync with your CRM, calendar, and team for zero-lag follow-up.",
    "Sincronização instantânea com seu CRM, calendário e equipe para acompanhamento sem atraso."
  ],
  [
    "Smart Escalation",
    "Escalonamento inteligente"
  ],
  [
    "Seamless handoffs to your team exactly when a human touch is needed.",
    "Repasse fluido para sua equipe exatamente quando o toque humano é necessário."
  ],
  [
    "SMS Terms",
    "Termos de SMS"
  ],
  [
    "Data Handling Notice",
    "Aviso de Tratamento de Dados"
  ],
  [
    "Responsible AI Policy",
    "Política de IA Responsável"
  ],
  [
    "Security Roadmap",
    "Roteiro de segurança"
  ],
  [
    "Legal",
    "Jurídico"
  ],
  [
    "Priority focus",
    "Foco prioritário"
  ],
  [
    "Managed partner",
    "Parceiro gerenciado"
  ],
  [
    "Support answers with approved context",
    "Respostas com contexto aprovado"
  ],
  [
    "Lead intake across WhatsApp, chat, SMS, forms, and email",
    "Captação de leads por WhatsApp, chat, SMS, formulários e e-mail"
  ],
  [
    "CRM, calendar, and team follow-up workflows",
    "Fluxos de trabalho de CRM, calendário e acompanhamento de equipe"
  ],
  [
    "Human review and escalation when it matters",
    "Revisão humana e escalonamento quando for importante"
  ],
  [
    "Customer conversations connected to business operations",
    "Conversas com clientes conectadas às operações de negócios"
  ],
  [
    "Team alerts",
    "Alertas de equipe"
  ],
  [
    "Customer demand grows faster than manual follow-up can keep up.",
    "A demanda do cliente cresce mais rápido do que o acompanhamento manual consegue acompanhar."
  ],
  [
    "For many SMBs, the issue is not effort. It is that conversations, customer details, and next steps are spread across busy people and disconnected tools.",
    "Para muitas PMEs, o problema não é falta de esforço. É que conversas, dados dos clientes e próximos passos ficam espalhados entre pessoas ocupadas e ferramentas desconectadas."
  ],
  [
    "DBX Solutions designs customer conversation systems that collect the right information, trigger the right workflow, and give your team useful context.",
    "A DBX Solutions cria sistemas de conversa com clientes que coletam as informações certas, acionam o fluxo correto e dão contexto útil à sua equipe."
  ],
  [
    "The result is a practical bridge between customer communication and the work your team already needs to do.",
    "O resultado é uma ponte prática entre a comunicação com o cliente e o trabalho que sua equipe já precisa realizar."
  ],
  [
    "Customers reach out through the channels they already use.",
    "Os clientes entram em contato por meio dos canais que já utilizam."
  ],
  [
    "The agent answers common questions, qualifies intent, collects details, triggers follow-up, and updates your tools when needed.",
    "O agente responde a perguntas comuns, qualifica a intenção, coleta detalhes, aciona o acompanhamento e atualiza suas ferramentas quando necessário."
  ],
  [
    "The assistant answers common questions, qualifies intent, collects details, and routes the next step.",
    "O assistente responde a perguntas comuns, qualifica a intenção, coleta detalhes e encaminha a próxima etapa."
  ],
  [
    "Useful customer context moves into the tools your team uses to manage work.",
    "O contexto útil do cliente passa para as ferramentas que sua equipe usa para gerenciar o trabalho."
  ],
  [
    "People stay in control for follow-up, exceptions, decisions, and customer relationships.",
    "As pessoas permanecem no controle do acompanhamento, das exceções, das decisões e do relacionamento com os clientes."
  ],
  [
    "Each service starts with a clear operational need, then turns it into a customer-facing workflow your team can manage.",
    "Cada serviço começa com uma necessidade operacional clara e depois o transforma em um fluxo de trabalho voltado para o cliente que sua equipe pode gerenciar."
  ],
  [
    "Customer-facing assistants for WhatsApp, web chat, SMS, forms, and email that answer common questions and guide people to the right next step.",
    "Assistentes voltados para o cliente para WhatsApp, chat na web, SMS, formulários e e-mail que respondem a perguntas comuns e orientam as pessoas para o próximo passo certo."
  ],
  [
    "Businesses that need dependable first-response coverage without adding complexity.",
    "Empresas que precisam de cobertura confiável de primeira resposta sem aumentar a complexidade."
  ],
  [
    "Clearer conversations and faster next steps, with handoff paths when people need to step in.",
    "Conversas mais claras e próximas etapas mais rápidas, com caminhos de transferência quando as pessoas precisam intervir."
  ],
  [
    "Structured intake flows that ask the right questions, capture buyer details, qualify fit, and route sales-ready leads with useful context.",
    "Fluxos de entrada estruturados que fazem as perguntas certas, capturam detalhes do comprador, qualificam a adequação e encaminham leads prontos para vendas com contexto útil."
  ],
  [
    "Teams receiving inquiries through WhatsApp, chat, forms, email, or booking requests.",
    "Equipes que recebem consultas por WhatsApp, chat, formulários, e-mail ou solicitações de reserva."
  ],
  [
    "More organized follow-up and cleaner information for sales conversations.",
    "Acompanhamento mais organizado e informações mais limpas para conversas de vendas."
  ],
  [
    "Handle common questions, support intake, ticket routing, status updates, and escalation paths for routine customer needs.",
    "Lide com perguntas comuns, entrada de suporte, encaminhamento de tickets, atualizações de status e caminhos de escalonamento para necessidades rotineiras do cliente."
  ],
  [
    "Support teams spending too much time on repeat requests and intake triage.",
    "As equipes de suporte gastam muito tempo em solicitações repetidas e triagem de captação."
  ],
  [
    "More consistent support coverage while the team focuses on higher-value issues.",
    "Cobertura de suporte mais consistente enquanto a equipe se concentra em questões de maior valor."
  ],
  [
    "Smoother handoffs, fewer manual updates, and better visibility into customer activity.",
    "Transferências mais tranquilas, menos atualizações manuais e melhor visibilidade da atividade do cliente."
  ],
  [
    "Connect customer conversations with CRM records, calendars, support desks, internal notifications, and operational workflows.",
    "Conecte conversas de clientes com registros de CRM, calendários, centrais de suporte, notificações internas e fluxos de trabalho operacionais."
  ],
  [
    "Businesses using multiple tools that do not communicate effectively.",
    "Empresas que usam múltiplas ferramentas que não se comunicam de forma eficaz."
  ],
  [
    "Review your customer journey, tools, team capacity, and operational goals to identify the best practical AI starting points.",
    "Revise a jornada do cliente, as ferramentas, a capacidade da equipe e as metas operacionais para identificar os melhores pontos de partida práticos para IA."
  ],
  [
    "Businesses that want AI clarity before investing in implementation.",
    "Empresas que desejam clareza sobre IA antes de investir na implementação."
  ],
  [
    "A focused roadmap tied to customer experience and operational value.",
    "Um roteiro focado vinculado à experiência do cliente e ao valor operacional."
  ],
  [
    "Review real conversations, improve responses, tune workflows, and expand useful improvements as your business changes.",
    "Revise conversas reais, melhore as respostas, ajuste os fluxos de trabalho e expanda melhorias úteis à medida que seu negócio muda."
  ],
  [
    "Review real customer interactions to spot unclear answers, missed intent, and handoff improvements.",
    "Revise as interações reais dos clientes para detectar respostas pouco claras, intenções perdidas e melhorias na transferência."
  ],
  [
    "Tune approved responses so agentic AI stays accurate, useful, and aligned with your business.",
    "Ajuste as respostas aprovadas para que a IA agéntica permaneça precisa, útil e alinhada com seu negócio."
  ],
  [
    "Workflow monitoring",
    "Monitoramento de fluxo de trabalho"
  ],
  [
    "Track where workflows help, stall, or need adjustment as volume and customer behavior change.",
    "Acompanhe onde os fluxos de trabalho ajudam, paralisam ou precisam de ajuste à medida que o volume e o comportamento do cliente mudam."
  ],
  [
    "Automation optimization",
    "Otimização de automação"
  ],
  [
    "Refine intake steps, routing rules, and follow-up triggers based on what customers actually do.",
    "Refine as etapas de entrada, as regras de roteamento e os gatilhos de acompanhamento com base no que os clientes realmente fazem."
  ],
  [
    "Keep AI useful as your business changes.",
    "Mantenha a IA útil à medida que seu negócio muda."
  ],
  [
    "AI systems require ongoing review, tuning, monitoring, and refinement. DBX helps keep agentic AI accurate, useful, and aligned with evolving workflows and customer needs.",
    "Os sistemas de IA exigem revisão, ajuste, monitoramento e refinamento contínuos. O DBX ajuda a manter a IA agéntica precisa, útil e alinhada com os fluxos de trabalho em evolução e as necessidades dos clientes."
  ],
  [
    "AI systems require ongoing review, tuning, monitoring, and refinement. DBX helps keep assistants accurate, useful, and aligned with evolving workflows and customer needs.",
    "Os sistemas de IA exigem revisão, ajuste, monitoramento e refinamento contínuos. O DBX ajuda a manter os assistentes precisos, úteis e alinhados com os fluxos de trabalho em evolução e as necessidades dos clientes."
  ],
  [
    "DBX guides each project through a clear, low-risk process that starts with your customer journey and ends with ongoing operational improvement.",
    "A DBX orienta cada projeto através de um processo claro e de baixo risco que começa com a jornada do cliente e termina com a melhoria operacional contínua."
  ],
  [
    "A practical consultation keeps the first implementation focused.",
    "Uma consulta prática mantém o foco na primeira implementação."
  ],
  [
    "Managed AI Operations gives your business ongoing review, tuning, and improvement as conversations and workflows evolve.",
    "As operações gerenciadas de IA oferecem ao seu negócio revisão, ajuste e melhorias contínuas à medida que as conversas e os fluxos de trabalho evoluem."
  ],
  [
    "Keep your AI assistant useful after launch.",
    "Mantenha seu assistente de IA útil após o lançamento."
  ],
  [
    "Agentic AI stays accurate, useful, and aligned with customer behavior.",
    "A IA agéntica permanece precisa, útil e alinhada com o comportamento do cliente."
  ],
  [
    "AI works best when it is monitored, measured, and improved.",
    "A IA funciona melhor quando é monitorada, medida e melhorada."
  ],
  [
    "DBX focuses on business outcomes that make customer communication clearer, follow-up easier, and make support more scalable.",
    "A DBX se concentra em resultados de negócios que tornam a comunicação com o cliente mais clara, o acompanhamento mais fácil e o suporte mais escalável."
  ],
  [
    "Give customers a useful answer or next step while intent is still high.",
    "Dê aos clientes uma resposta útil ou a próxima etapa enquanto a intenção ainda é alta."
  ],
  [
    "Capture and route inquiries before prospects lose interest or choose another provider.",
    "Capture e direcione consultas antes que os clientes potenciais percam o interesse ou escolham outro fornecedor."
  ],
  [
    "Move repetitive questions, intake, and routing out of your team’s daily queue.",
    "Remova perguntas repetitivas, entradas e encaminhamentos da fila diária de sua equipe."
  ],
  [
    "Keep answers clear, approved, and aligned across channels and team handoffs.",
    "Mantenha as respostas claras, aprovadas e alinhadas entre canais e transferências de equipe."
  ],
  [
    "Turn conversations into structured information your team can review and act on.",
    "Transforme conversas em informações estruturadas que sua equipe possa analisar e agir."
  ],
  [
    "Handle more routine conversations while your team focuses on higher-value work.",
    "Lide com conversas mais rotineiras enquanto sua equipe se concentra em trabalhos de maior valor."
  ],
  [
    "We review your customer journey, current tools, common customer interactions, team workflows, and business goals.",
    "Analisamos a jornada do cliente, as ferramentas atuais, as interações comuns com os clientes, os fluxos de trabalho da equipe e as metas de negócios."
  ],
  [
    "Identify customer experience gaps",
    "Identifique lacunas na experiência do cliente"
  ],
  [
    "Review repetitive tasks",
    "Revise tarefas repetitivas"
  ],
  [
    "Understand current systems",
    "Entenda os sistemas atuais"
  ],
  [
    "Define business priorities",
    "Defina prioridades de negócios"
  ],
  [
    "We map the right AI experience, conversation flows, automations, integrations, and success metrics.",
    "Mapeamos a experiência certa de IA, fluxos de conversa, automações, integrações e métricas de sucesso."
  ],
  [
    "Design customer interaction flows",
    "Projete fluxos de interação com o cliente"
  ],
  [
    "Define automation logic",
    "Definir lógica de automação"
  ],
  [
    "Plan handoffs to your team",
    "Planeje transferências para sua equipe"
  ],
  [
    "Align with your brand voice",
    "Alinhe-se com a voz da sua marca"
  ],
  [
    "We build, configure, test, and launch the solution using your existing tools, workflows, and brand guidelines.",
    "Construímos, configuramos, testamos e lançamos a solução usando suas ferramentas, fluxos de trabalho e diretrizes de marca existentes."
  ],
  [
    "Configure agentic AI",
    "Configurar IA agéntica"
  ],
  [
    "Configure AI assistants",
    "Configurar assistentes de IA"
  ],
  [
    "Connect systems where needed",
    "Conecte sistemas onde necessário"
  ],
  [
    "Test conversation quality",
    "Teste a qualidade da conversa"
  ],
  [
    "Prepare your team for launch",
    "Prepare sua equipe para o lançamento"
  ],
  [
    "We monitor performance, review user behavior, improve flows, and refine the solution as your business evolves.",
    "Monitoramos o desempenho, analisamos o comportamento dos usuários, melhoramos os fluxos e refinamos a solução à medida que seu negócio evolui."
  ],
  [
    "Review performance data",
    "Revise os dados de desempenho"
  ],
  [
    "Improve responses and flows",
    "Melhore as respostas e os fluxos"
  ],
  [
    "Adjust automation rules",
    "Ajuste as regras de automação"
  ],
  [
    "Identify new opportunities",
    "Identifique novas oportunidades"
  ],
  [
    "Map your first AI workflow with DBX.",
    "Mapeie seu primeiro fluxo de trabalho de IA com DBX."
  ],
  [
    "Bring your current customer channels, common questions, and follow-up process. We will help turn them into a practical implementation path.",
    "Traga seus canais atuais de clientes, dúvidas comuns e processo de acompanhamento. Ajudaremos a transformá-los em um caminho de implementação prático."
  ],
  [
    "Guided adoption makes AI feel lower-risk and easier to manage.",
    "A adoção guiada faz com que a IA pareça menos arriscada e mais fácil de gerenciar."
  ],
  [
    "Explore the first customer workflow worth improving.",
    "Explore o primeiro fluxo de trabalho do cliente que vale a pena melhorar."
  ],
  [
    "We can help identify where customer response, intake, qualification, or follow-up should become clearer first.",
    "Podemos ajudar a identificar onde a resposta, a captação, a qualificação ou o acompanhamento do cliente devem ficar mais claros primeiro."
  ],
  [
    "DBX helps SMBs design, connect, monitor, and improve AI-assisted customer conversation systems around real business workflows.",
    "DBX ajuda pequenas e médias empresas a projetar, conectar, monitorar e melhorar sistemas de conversação com clientes assistidos por IA em torno de fluxos de trabalho de negócios reais."
  ],
  [
    "DBX Solutions helps SMBs improve customer conversations, qualify leads, automate repetitive work, and connect AI with real business operations.",
    "A DBX Solutions ajuda as pequenas e médias empresas a melhorar as conversas com os clientes, qualificar leads, automatizar o trabalho repetitivo e conectar a IA com operações comerciais reais."
  ],
  [
    "Designed for growing businesses that want clearer customer communication and practical AI implementation.",
    "Projetado para empresas em crescimento que desejam uma comunicação mais clara com o cliente e implementação prática de IA."
  ],
  [
    "Focused on SMB sectors where conversations drive revenue and service quality.",
    "Focado em setores de pequenas e médias empresas onde as conversas geram receita e qualidade de serviço."
  ],
  [
    "DBX is especially useful for businesses that depend on fast inquiry handling, clear intake, reliable follow-up, and repeatable customer support.",
    "O DBX é especialmente útil para empresas que dependem de tratamento rápido de consultas, entrada clara, acompanhamento confiável e suporte ao cliente repetível."
  ],
  [
    "Automate intake, qualify inquiries, route prospects, and improve response times for consulting, legal, financial, and advisory teams.",
    "Automatize a captação, qualifique consultas, encaminhe prospects e melhore os tempos de resposta para equipes de consultoria, jurídicas, financeiras e de assessoria."
  ],
  [
    "Support appointment inquiries, FAQs, intake guidance, service information, and patient communication workflows.",
    "Atenda consultas de agendamento, FAQs, orientações de captação, informações de serviço e fluxos de comunicação com pacientes."
  ],
  [
    "Qualify buyer and seller inquiries, answer common property questions, route leads, and support faster follow-up.",
    "Qualifique as consultas de compradores e vendedores, responda a perguntas comuns sobre propriedades, encaminhe leads e ofereça suporte a um acompanhamento mais rápido."
  ],
  [
    "Automate quote requests, booking inquiries, service questions, and customer follow-ups for high-message-volume teams.",
    "Automatize solicitações de orçamento, consultas de reserva, perguntas sobre serviços e acompanhamento de clientes para equipes com alto volume de mensagens."
  ],
  [
    "Support product questions, order-related inquiries, return guidance, and routine customer service requests.",
    "Suporte a dúvidas sobre produtos, consultas relacionadas a pedidos, orientações de devolução e solicitações rotineiras de atendimento ao cliente."
  ],
  [
    "Answer program questions, qualify student or client interest, guide enrollment inquiries, and automate follow-up.",
    "Responda a perguntas do programa, qualifique o interesse de alunos ou clientes, oriente consultas de matrícula e automatize o acompanhamento."
  ],
  [
    "Improve inquiry handling, appointment requests, client intake, and customer communication with human oversight.",
    "Melhore o tratamento de consultas, solicitações de agendamento, captação de clientes e comunicação com o cliente com supervisão humana."
  ],
  [
    "Solutions are designed for growing businesses that need practical value without enterprise-level complexity.",
    "As soluções são projetadas para empresas em crescimento que precisam de valor prático sem complexidade de nível empresarial."
  ],
  [
    "We start with customer journeys, operational challenges, and business goals before recommending technology.",
    "Começamos com as jornadas do cliente, os desafios operacionais e as metas de negócios antes de recomendar a tecnologia."
  ],
  [
    "AI supports your team while escalation, review, and judgment remain available when needed.",
    "A IA apoia sua equipe enquanto o escalonamento, a revisão e o critério humano permanecem disponíveis quando necessário."
  ],
  [
    "We consider how AI fits with your existing channels, workflows, CRM, and customer communication tools.",
    "Consideramos como a IA se adapta aos seus canais, fluxos de trabalho, CRM e ferramentas de comunicação com o cliente existentes."
  ],
  [
    "Every solution connects to a business outcome such as better lead handling, support consistency, or reduced manual work.",
    "Cada solução se conecta a um resultado de negócios, como melhor tratamento de leads, consistência de suporte ou redução de trabalho manual."
  ],
  [
    "We help refine flows, responses, and automations as your business learns what works best.",
    "Ajudamos a refinar fluxos, respostas e automações à medida que sua empresa aprende o que funciona melhor."
  ],
  [
    "Agentic AI is built around approved business information, clear rules, escalation paths, and ongoing quality review.",
    "A IA agéntica é construída com base em informações comerciais aprovadas, regras claras, caminhos de escalonamento e revisão contínua de qualidade."
  ],
  [
    "Assistants are built around approved business information, clear rules, escalation paths, and ongoing quality review.",
    "Os assistentes são criados com base em informações comerciais aprovadas, regras claras, caminhos de escalonamento e revisão contínua da qualidade."
  ],
  [
    "Yes. WhatsApp can be part of the channel mix along with web chat, SMS, forms, and email, depending on your setup.",
    "Sim. O WhatsApp pode fazer parte do mix de canais junto com chat na web, SMS, formulários e e-mail, dependendo da sua configuração."
  ],
  [
    "Yes. DBX designs escalation paths so your team can review, respond, or take over when a conversation needs human judgment.",
    "Sim. A DBX projeta caminhos de escalonamento para que sua equipe possa revisar, responder ou assumir o controle quando uma conversa precisar de critério humano."
  ],
  [
    "Timing depends on scope, channels, and integrations. Many businesses start with one focused use case, then expand after launch.",
    "O tempo depende do escopo, canais e integrações. Muitas empresas começam com um caso de uso focado e expandem após o lançamento."
  ],
  [
    "Can DBX connect with our existing tools?",
    "O DBX pode se conectar com nossas ferramentas existentes?"
  ],
  [
    "In many cases, yes. DBX can connect conversations with CRMs, calendars, support systems, forms, notifications, and workflow tools.",
    "Em muitos casos, sim. DBX pode conectar conversas com CRMs, calendários, sistemas de suporte, formulários, notificações e ferramentas de fluxo de trabalho."
  ],
  [
    "No. The goal is to support your team, reduce repetitive work, and keep people focused on higher-value customer needs.",
    "Não. O objetivo é apoiar sua equipe, reduzir o trabalho repetitivo e manter as pessoas focadas nas necessidades de maior valor do cliente."
  ],
  [
    "DBX Solutions helps small and mid-sized businesses turn customer conversations into clearer support, qualified leads, and connected follow-up workflows.",
    "A DBX Solutions ajuda pequenas e médias empresas a transformar conversas com clientes em suporte mais claro, leads qualificados e fluxos de trabalho de acompanhamento conectados."
  ],
  [
    "Practical solutions for WhatsApp, intake, and connected follow-up",
    "Soluções práticas para WhatsApp, captação e acompanhamento conectado"
  ],
  [
    "DBX helps growing businesses turn WhatsApp and other customer channels into faster responses, clearer lead intake, and workflows your team can act on—without adding headcount.",
    "A DBX ajuda empresas em crescimento a transformar o WhatsApp e outros canais de clientes em respostas mais rápidas, captação de leads mais clara e fluxos de trabalho nos quais sua equipe pode atuar, sem aumentar o número de funcionários."
  ],
  [
    "Services that make WhatsApp and customer AI practical",
    "Serviços que tornam prático o WhatsApp e a IA do cliente"
  ],
  [
    "From agentic AI on WhatsApp and chat to CRM integration and ongoing optimization—DBX implements services around how your team already works.",
    "Desde IA agéntica no WhatsApp e chat até integração de CRM e otimização contínua, o DBX implementa serviços de acordo com o modo como sua equipe já trabalha."
  ],
  [
    "Where WhatsApp and customer conversations drive revenue",
    "Onde o WhatsApp e as conversas com os clientes geram receita"
  ],
  [
    "DBX helps growing businesses in service-heavy sectors respond faster on WhatsApp, qualify inquiries, and keep follow-up connected to the tools teams already use.",
    "A DBX ajuda empresas em crescimento em setores com muitos serviços a responder mais rapidamente no WhatsApp, qualificar consultas e manter o acompanhamento conectado às ferramentas que as equipes já usam."
  ],
  [
    "Helping growing businesses turn AI into better customer experiences",
    "Ajudando empresas em crescimento a transformar IA em melhores experiências para o cliente"
  ],
  [
    "DBX Solutions was built to help small and mid-sized businesses adopt AI in a practical, human-centered, and business-focused way—with WhatsApp and customer channels at the center.",
    "A DBX Solutions foi criada para ajudar pequenas e médias empresas a adotar a IA de uma forma prática, centrada no ser humano e nos negócios, com o WhatsApp e os canais do cliente no centro."
  ],
  [
    "Want to see how this applies to your business?",
    "Quer ver como isso se aplica ao seu negócio?"
  ],
  [
    "Book a free consultation to review your channels, workflows, and the first practical opportunity to improve.",
    "Agende uma consulta gratuita para revisar seus canais, fluxos de trabalho e a primeira oportunidade prática de melhoria."
  ],
  [
    "No pressure—just a focused conversation about what would help your team most.",
    "Sem pressão – apenas uma conversa focada sobre o que mais ajudaria sua equipe."
  ],
  [
    "DBX focuses on measurable outcomes—response speed, cleaner intake, and follow-up your team can use.",
    "A DBX foca em resultados mensuráveis—velocidade de resposta, captação mais clara e acompanhamento que sua equipe pode usar."
  ],
  [
    "Outcomes vary by channel volume, workflow scope, and how systems connect to your team.",
    "Os resultados variam de acordo com o volume do canal, o escopo do fluxo de trabalho e como os sistemas se conectam à sua equipe."
  ],
  [
    "Illustrative scenario",
    "Cenário ilustrativo"
  ],
  [
    "Service business with WhatsApp intake",
    "Negócio de serviços com captação de WhatsApp"
  ],
  [
    "Organizing WhatsApp inquiries gave the team useful context before follow-up. Responses got faster without adding headcount.",
    "A organização das consultas do WhatsApp deu à equipe um contexto útil antes do acompanhamento. As respostas ficaram mais rápidas sem aumentar o número de funcionários."
  ],
  [
    "WhatsApp, intake, and follow-up workflows.",
    "Fluxos de trabalho de WhatsApp, entrada e acompanhamento."
  ],
  [
    "Implementation tied to your channels and CRM.",
    "Implementação atrelada aos seus canais e CRM."
  ],
  [
    "Sector examples—not generic automation.",
    "Exemplos setoriais – não automação genérica."
  ],
  [
    "Calendar for speed, form for a short note.",
    "Calendário para velocidade, formulário para uma nota curta."
  ],
  [
    "Approved data sources and controlled access.",
    "Fontes de dados aprovadas e acesso controlado."
  ],
  [
    "Escalation paths and clear AI limits.",
    "Caminhos de escalonamento e limites claros de IA."
  ],
  [
    "Roadmap items—not claimed certifications.",
    "Itens do roteiro – certificações não reivindicadas."
  ],
  [
    "Data handling",
    "Tratamento de dados"
  ],
  [
    "DBX designs AI workflows around approved business information, controlled access, and clear limits on what systems process and store.",
    "A DBX projeta fluxos de trabalho de IA em torno de informações comerciais aprovadas, acesso controlado e limites claros sobre o que os sistemas processam e armazenam."
  ],
  [
    "DBX implements agentic AI with clear escalation paths, approved business information, and human review—not unconstrained automation.",
    "O DBX implementa IA agéntica com caminhos de escalonamento claros, informações comerciais aprovadas e revisão humana, e não automação irrestrita."
  ],
  [
    "DBX aligns operations with recognized security and responsible AI practices. Items below are roadmap or readiness work unless explicitly marked complete.",
    "DBX alinha operações com segurança reconhecida e práticas responsáveis de IA. Os itens abaixo são roteiro ou trabalho de preparação, a menos que estejam explicitamente marcados como concluídos."
  ],
  [
    "Our approach",
    "Nossa abordagem"
  ],
  [
    "What we prioritize",
    "O que priorizamos"
  ],
  [
    "Working principles",
    "Princípios de funcionamento"
  ],
  [
    "Security roadmap",
    "Roteiro de segurança"
  ],
  [
    "Roadmap / In progress",
    "Roteiro / Em andamento"
  ],
  [
    "Security and compliance roadmap",
    "Roteiro de segurança e conformidade"
  ],
  [
    "By checking this box, you provide express written consent to receive SMS communications from DBX Solutions LLC related to appointments, services, and support. Message frequency varies. Message and data rates may apply. Consent is not a condition of purchase. Reply STOP to opt out and HELP for help. See Privacy Policy and SMS Terms.",
    "Ao marcar esta caixa, você fornece consentimento expresso por escrito para receber comunicações SMS da DBX Solutions LLC relacionadas a compromissos, serviços e suporte. A frequência das mensagens varia. Podem ser cobradas tarifas de mensagens e dados. O consentimento não é uma condição de compra. Responda STOP para cancelar e HELP para obter ajuda. Consulte a Política de Privacidade e os Termos de SMS."
  ],
  [
    "Example: A firm captures project scope and timeline on WhatsApp, then routes qualified inquiries to the right advisor with full context.",
    "Exemplo: uma empresa captura o escopo e o cronograma do projeto no WhatsApp e, em seguida, encaminha consultas qualificadas para o consultor certo com contexto completo."
  ],
  [
    "Example: After-hours WhatsApp questions about hours, services, and booking get answered while urgent cases escalate to staff.",
    "Exemplo: perguntas do WhatsApp fora do horário comercial sobre horários, serviços e reservas são respondidas enquanto casos urgentes são encaminhados para a equipe."
  ],
  [
    "Example: A buyer asks about availability on WhatsApp; the agent collects budget and timeline before alerting the listing agent.",
    "Exemplo: um comprador pergunta sobre disponibilidade no WhatsApp; o agente coleta orçamento e cronograma antes de alertar o corretor responsável pelo imóvel."
  ],
  [
    "Example: Quote requests include service type, location, and photos so your team replies with accurate next steps, not repeat questions.",
    "Exemplo: as solicitações de orçamento incluem tipo de serviço, localização e fotos para que sua equipe responda com as próximas etapas precisas, sem repetir perguntas."
  ],
  [
    "Example: Order-status questions on WhatsApp pull structured details before a human steps in for exceptions.",
    "Exemplo: perguntas sobre o status do pedido no WhatsApp extraem detalhes estruturados antes que um humano intervenha para exceções."
  ],
  [
    "Example: Program FAQs and intake questions run on chat while enrollment-ready leads land in your CRM with notes.",
    "Exemplo: FAQs do programa e perguntas de captação rodam no chat enquanto leads prontos para inscrição chegam ao seu CRM com notas."
  ],
  [
    "Example: General service questions are handled consistently; sensitive or compliance-related topics route to a human reviewer.",
    "Exemplo: As questões gerais de serviço são tratadas de forma consistente; tópicos sensíveis ou relacionados à conformidade são encaminhados para um revisor humano."
  ],
  [
    "Agentic AI for WhatsApp, web chat, SMS, forms, and email—responds, qualifies, collects data, and routes follow-up to your team or CRM.",
    "IA agéntica para WhatsApp, chat na web, SMS, formulários e e-mail—responde, qualifica, coleta dados e encaminha o acompanhamento para sua equipe ou CRM."
  ],
  [
    "WhatsApp · Chat · Email",
    "WhatsApp · Bate-papo · E-mail"
  ],
  [
    "Team inbox",
    "Caixa de entrada da equipe"
  ],
  [
    "CRM",
    "CRM"
  ],
  [
    "Slow response",
    "Resposta lenta"
  ],
  [
    "Lost context",
    "Contexto perdido"
  ],
  [
    "Missed lead",
    "Lead perdido"
  ],
  [
    "Customer frustration",
    "Frustração do cliente"
  ],
  [
    "Slow or inconsistent replies erode trust—customers feel ignored even when your team is doing their best behind the scenes.",
    "Respostas lentas ou inconsistentes prejudicam a confiança – os clientes se sentem ignorados mesmo quando sua equipe está fazendo o melhor nos bastidores."
  ],
  [
    "Customer messages turn into team action—without the manual chaos.",
    "As mensagens dos clientes se transformam em ações de equipe, sem o caos manual."
  ],
  [
    "DBX connects how customers reach you with what your team needs to do next.",
    "DBX conecta como os clientes chegam até você com o que sua equipe precisa fazer a seguir."
  ],
  [
    "They message you",
    "Eles mandam mensagens para você"
  ],
  [
    "On WhatsApp, chat, and email—where they already are.",
    "No WhatsApp, chat e e-mail – onde eles já estão."
  ],
  [
    "DBX answers first",
    "DBX responde primeiro"
  ],
  [
    "Common questions handled with your business context, day or night.",
    "Perguntas comuns tratadas no contexto do seu negócio, dia ou noite."
  ],
  [
    "Your tools stay current",
    "Suas ferramentas permanecem atualizadas"
  ],
  [
    "CRM, calendar, and team alerts updated—no retyping.",
    "Alertas de CRM, calendário e equipe atualizados – sem redigitação."
  ],
  [
    "Your team stays in control",
    "Sua equipe permanece no controle"
  ],
  [
    "People step in for judgment calls, follow-up, and closing.",
    "As pessoas intervêm para critério humano, acompanhamento e encerramento."
  ],
  [
    "Connected customer-to-team workflow",
    "Fluxo de trabalho conectado entre cliente e equipe"
  ],
  [
    "DBX handles intake",
    "DBX gerencia a entrada"
  ],
  [
    "Your team in the loop",
    "Sua equipe informada"
  ],
  [
    "Faster reply",
    "Resposta mais rápida"
  ],
  [
    "Clear handoff",
    "Transferência clara"
  ],
  [
    "No duplicate work",
    "Nenhum trabalho duplicado"
  ],
  [
    "Human-supervised. Your tools. Your rules.",
    "Supervisionado por humanos. Suas ferramentas. Suas regras."
  ],
  [
    "Chat",
    "Chat"
  ],
  [
    "How DBX helps your business every day",
    "Como o DBX ajuda sua empresa no dia a dia"
  ],
  [
    "Tell us where you're stuck—we connect WhatsApp, your team, and the tools you already use.",
    "Diga onde você trava—conectamos WhatsApp, sua equipe e as ferramentas que você já usa."
  ],
  [
    "How DBX helps from first message to team action",
    "Como o DBX ajuda da primeira mensagem à ação da equipe"
  ],
  [
    "They write you",
    "Clientes escrevem para você"
  ],
  [
    "DBX replies",
    "DBX responde"
  ],
  [
    "You get the details",
    "Você recebe os dados"
  ],
  [
    "Your tools update",
    "Suas ferramentas atualizam"
  ],
  [
    "Your team closes",
    "Sua equipe fecha"
  ],
  [
    "Good if you",
    "Ideal se"
  ],
  [
    "What changes",
    "O que melhora"
  ],
  [
    "Start here",
    "Comece aqui"
  ],
  [
    "We run it with you",
    "Operamos com você"
  ],
  [
    "Reply on WhatsApp and chat",
    "Respostas no WhatsApp e no chat"
  ],
  [
    "Answer common questions fast, even after hours. Your team takes over when a person needs to decide.",
    "Responda perguntas comuns rapidamente, mesmo depois do expediente. Sua equipe assume quando uma pessoa precisa decidir."
  ],
  [
    "Customers write you and answers take too long.",
    "Os clientes escrevem para você e as respostas demoram muito."
  ],
  [
    "Fewer missed messages. Clearer next steps.",
    "Menos mensagens perdidas. Próximos passos mais claros."
  ],
  [
    "Sort leads before your team follows up",
    "Classifique os leads antes que sua equipe faça o acompanhamento"
  ],
  [
    "Ask the right questions first so your team calls back with context—not from scratch.",
    "Faça as perguntas certas primeiro para que sua equipe retorne a ligação contextualizada, e não do zero."
  ],
  [
    "You get inquiries but follow-up feels messy.",
    "Você recebe perguntas, mas o acompanhamento parece confuso."
  ],
  [
    "Better leads, less back-and-forth.",
    "Melhores leads, menos idas e vindas."
  ],
  [
    "Handle repeat questions without burning out your team",
    "Lide com perguntas repetidas sem esgotar sua equipe"
  ],
  [
    "Common support questions get a consistent answer. Tricky cases go to a person.",
    "Perguntas comuns de suporte recebem uma resposta consistente. Casos complicados vão para uma pessoa."
  ],
  [
    "Your team answers the same things all day.",
    "Sua equipe responde às mesmas coisas o dia todo."
  ],
  [
    "More time for real problems.",
    "Mais tempo para problemas reais."
  ],
  [
    "Update your CRM without retyping everything",
    "Atualize seu CRM sem redigitar tudo"
  ],
  [
    "What the customer said shows up in your CRM, calendar, or team alerts—automatically.",
    "O que o cliente disse aparece em seu CRM, calendário ou alertas de equipe – automaticamente."
  ],
  [
    "Info lives in chats and never makes it to your tools.",
    "As informações ficam nos chats e nunca chegam às suas ferramentas."
  ],
  [
    "Less duplicate work. Everyone sees the same picture.",
    "Menos trabalho duplicado. Todo mundo vê a mesma imagem."
  ],
  [
    "Know where to start with AI",
    "Saiba por onde começar com IA"
  ],
  [
    "We look at how customers reach you today and pick practical first steps—no big vague project.",
    "Observamos como os clientes chegam até você hoje e escolhemos os primeiros passos práticos – sem grandes projetos vagos."
  ],
  [
    "You want AI help but do not know what to do first.",
    "Você quer ajuda de IA, mas não sabe o que fazer primeiro."
  ],
  [
    "A clear plan you can act on.",
    "Um plano claro no qual você pode agir."
  ],
  [
    "We keep improving it with you",
    "Continuamos melhorando com você"
  ],
  [
    "After launch, we review real conversations and adjust so it stays useful as your business changes.",
    "Após o lançamento, revisamos conversas reais e as ajustamos para que continuem úteis à medida que seu negócio muda."
  ],
  [
    "You do not want a one-time setup that goes stale.",
    "Você não quer uma configuração única que fique obsoleta."
  ],
  [
    "Answers stay useful. Processes stay up to date.",
    "As respostas continuam úteis. Os processos permanecem atualizados."
  ]
]
